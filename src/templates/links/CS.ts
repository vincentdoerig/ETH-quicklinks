import { nanoid } from 'nanoid'

export default [
  {
    id: nanoid(5),
    label: 'myStudies',
    href: 'https://www.lehrbetrieb.ethz.ch/myStudies/studWillkommen.view',
  },
  {
    id: nanoid(5),
    label: 'Live Lectures',
    href: 'https://video.ethz.ch/live/lectures/zentrum.html',
  },
  {
    id: nanoid(5),
    label: 'Past Lectures',
    href: 'https://video.ethz.ch/lectures/d-infk/2022/autumn.html',
  },
  {
    id: nanoid(5),
    label: 'VIS Exam Collection',
    href: 'https://exams.vis.ethz.ch/',
  },
  {
    id: nanoid(5),
    label: 'PVW Skripts',
    href: 'https://vis.ethz.ch/de/services/pvw-scripts/',
  },
  {
    id: nanoid(5),
    label: 'Vorlesungsverzeichnis',
    href: 'http://vvz.ethz.ch/Vorlesungsverzeichnis/sucheLehrangebot.view?lang=de&search=on&semkez=2022W&studiengangTyp=BSC&deptId=5&studiengangAbschnittId=&lerneinheitstitel=&lerneinheitscode=&famname=&rufname=&wahlinfo=&lehrsprache=&periodizitaet=&katalogdaten=&_strukturAus=on&search=Suchen',
  },
]
